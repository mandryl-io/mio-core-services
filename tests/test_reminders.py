...
def _kicked_diapex(pushed) -> bool:
    return any(
        isinstance(frame, LLMContextFrame)
        and any(
            isinstance(message, dict) and "Diapex" in str(message.get("content", ""))
...
async def test_due_dose_queues_context_the_llm_will_run():
    book = _book()
    pushed = []
